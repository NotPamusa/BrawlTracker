...
import { PlayerStats, RELEASED_BUFFIES_BRAWLER_COUNT } from "@/utils/brawlAPI";
...
export default function PlayerProgressionDashboard({ player, stats, tag }: PlayerProgressionDashboardProps) {
  const { settings, isApplying } = useSettings();
  const [hasAppliedTuning, setHasAppliedTuning] = useState(false);

  // When isApplying becomes true, it means the user clicked apply tuning at least once
  useEffect(() => {
    if (isApplying) setHasAppliedTuning(true);
  }, [isApplying]);
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-1 flex flex-col gap-4">
          <EstimateCard
            ...
            disableGlow={!hasAppliedTuning}
          />
          <CalculatorSettings player={player} />
        </div>
